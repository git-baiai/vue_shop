            ...
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0" :class="{ active: saleAttrValue.isChecked == 1 }"
                  v-for="saleAttrValue in spuSaleAttr.spuSaleAttrValueList" :key="saleAttrValue.id"
...
import router from '../../router'
  ...
  export default {
    ...
    setup() {
      onMounted(()=>{
        store.dispatch('goodInfo',router.currentRoute.value.params.skuid)
      })
      ...
      let skuImgList = computed(()=>{
        return skuInfo.value.skuImageList || []
      })
      let spuSaleAttrList = computed(()=>{
        return store.getters.spuSaleAttrList
      })
      let changeActive = function (value,arr) { 
        arr.forEach(item => {
          item.isChecked = '0'
        });
        value.isChecked = '1'
      ...
      let skuNum = ref(1)
      let changeSkuNum = function (event) { 
        let num = event.target.value * 1
        skuNum.value = (isNaN(num) || num < 1)? 1 : parseInt(num)
       }
      let addShopCar = async function () { 
        try{
          await store.dispatch('addOrUpdateShopCar', { skuId: router.currentRoute.value.params.skuid, skuNum: skuNum.value })
      ...
      return{
        categoryView, skuInfo, skuImgList, spuSaleAttrList,skuNum,
        changeActive,changeSkuNum,addShopCar
      }
    }
  }
</script>
