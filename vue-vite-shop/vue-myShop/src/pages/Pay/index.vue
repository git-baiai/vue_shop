  ...
  export default {
    ...
    setup() {
      ...
      let orderId = computed(() => {
        return router.currentRoute.value.query.orderId
      })
      let getPayInfo =async function () { 
        let result = await reqPayInfo(orderId.value)
        // console.log(result)
        if(result.code == 200){
          totalPrice.value = result.data.totalFee
          codeUrl.value = result.data.codeUrl
      ...
      let open = async function () { 
        let url = await QRcode.toDataURL(codeUrl.value)
        ElMessageBox.alert(
          `<img src=${url} />`,
          '请您微信扫码支付',
          {
      ...
      return{
        orderId,totalPrice,open,codeUrl
      }
    }
  }
</script>
